...
static bool CreateBuffers(Mesh* mesh)
{
	...
	if (FAILED(d3dDevice->lpVtbl->CreateVertexBuffer(d3dDevice, vertexBufferSize, 0, RENDERFVF, D3DPOOL_DEFAULT, &mesh->vertexBuffer, NULL)))
	{
		return false;
	}
	...
	if (FAILED(d3dDevice->lpVtbl->CreateIndexBuffer(d3dDevice, indexBufferSize, 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &mesh->indexBuffer, NULL)))
	{
		return false;
	}
